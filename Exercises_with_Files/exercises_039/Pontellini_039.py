...
#Pagina della flashcard (/flashcard/<id>):
#Quando l'utente visita l'URL, dovrebbe vedere una flashcard con la domanda.
...
#Quando l'utente invia la propria risposta tramite il modulo, l'applicazione verificherà se la risposta è corretta.
...
#Flashcard casuale:
#Aggiungi un pulsante "Casuale" che porta l'utente a una flashcard casuale ./flashcard/random
...
#Ricorda, questo è un approccio dall'alto verso il basso.
#Puoi iniziare a lavorare su ogni passaggio, testarlo a fondo prima di passare a quello successivo.
#In questo modo, è possibile assicurarsi che ogni parte dell'applicazione funzioni come previsto prima di aggiungere ulteriore complessità.
...
#app = Flask(__name__)
#Sostituire i prompt dell'interfaccia della riga di comando con route e viste Flask.
#Ogni route in un'applicazione Flask è associata a una funzione Python.
#Quando si accede al percorso tramite un browser Web, viene eseguita la funzione associata.
...
#Creare un file modello HTML denominato 'flashcard.html' in una cartella templates nella directory del progetto.
#Questo file deve contenere la struttura HTML della flashcard e del modulo, con segnaposto per i dati della flashcard.
...
#Il codice HTML fornito è un modello per la visualizzazione di una flashcard e un modulo per l'invio di una risposta.
#Il modello utilizza il linguaggio di template Jinja2 di Flask.

#<h1>{{ flashcard.question }}</h1>:

#Questa riga mostra la domanda della flashcard.
#È un segnaposto che viene sostituito con la domanda effettiva quando viene eseguito il rendering del modello.{{ flashcard.question }}

#<form action="/answer/{{ flashcard.id }}" method="post">:

#questa riga inizia un modulo.
#L'attributo specifica dove inviare i dati del modulo quando il modulo viene inviato. In questo caso, i dati del modulo vengono inviati all'URL.
#È un segnaposto che viene sostituito con l'ID effettivo della flashcard quando viene eseguito il rendering del modello.
#L'attributo specifica che il metodo HTTP POST deve essere utilizzato per l'invio dei dati del modulo.action/answer/{{ flashcard.id }}{{ flashcard.id }}method="post"
...
#<input type="submit" value="Submit">: questa riga crea un pulsante di invio.
#Quando l'utente fa clic su questo pulsante, i dati del modulo vengono inviati all'URL specificato nell'attributo.action
...
#Creare un file modello HTML denominato 'result.html' nella cartella dei modelli.
#Questo file dovrebbe visualizzare un messaggio in base al fatto che la risposta dell'utente sia corretta.
