...
class Buffer:
    def __init__(self):
        ...
        self.produttore_semaforo = threading.Semaphore(1)
        self.consumatore_semaforo = threading.Semaphore(0)

    def get_dato(self, name):
        self.consumatore_semaforo.acquire()
        dato = self.areaCondivisa
        print(f'{name} ha consumato il dato: {dato}')
        self.produttore_semaforo.release()
        return dato
        
    def set_dato(self, dato, name):
        self.produttore_semaforo.acquire()
        self.areaCondivisa = dato
        print(f'{name} ha prodotto il dato: {dato}')
        self.consumatore_semaforo.release()
