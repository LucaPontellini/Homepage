from Pontellini_007 import LibraryMaterial, Book, Magazine, DVD

def test_library_material():
    material = LibraryMaterial("Generic Material", 2020)
    assert material.get_title() == "Generic Material"
    assert material.get_year_of_publication() == 2020
...
def test_book():
    ...
    assert book.get_year_of_publication() == 1954
...
def test_magazine():
    ...
    assert magazine.get_year_of_publication() == 2023
    assert magazine.get_issue_number() == 5
    assert magazine.get_month_of_publication() == "May"
...
def test_dvd():
    ...
    assert dvd.get_year_of_publication() == 2010
...
def test_search():
    ...
    result = LibraryMaterial.search(materials, title="Inception")
    assert result.get_title() == "Inception" # type: ignore